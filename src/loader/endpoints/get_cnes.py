...
def get_leitos(url):
  ...
  element[0].click()
  df_leitos = pd.DataFrame(columns=['city_name', 'cirurgico_tot_ago', 'clinico_tot_ago','pediatrico_tot_ago','hospital_dia_tot_ago'])
  ...
  for j in range(4, len(y)):
    df_leitos.loc[i] = [x[k].text, x[k+1].text, x[k+2].text, x[k+4].text, x[k+7].text]
    k = k+8
    i = i+1
...
def get_urlleitoscomp(url):
  ...
  html = tableRows[0].get_attribute("innerHTML")
  soup = BeautifulSoup(html, "html.parser")
  df_leitos_comp = pd.DataFrame(columns=['city_name', 'UTI_adulto_I_tot_ago','UTI_adulto_II_tot_ago','UTI_adulto_III_tot_ago'])
  x = soup.select("td")
  y = soup.select("tr")
  i = 0
  k = 24
  ...
  html = tableRows[0].get_attribute("innerHTML")
  soup = BeautifulSoup(html, "html.parser")
  df_Leitos_compl_SUS_ago = pd.DataFrame(columns=['city_name','UTI_adulto_II_COVID_SUS_ago','UTI_pediatrica_II_COVID_SUS_ago',])
  x = soup.select("td")
  y = soup.select("tr")
  i = 0
  k = 24
  ...
  html = tableRows[0].get_attribute("innerHTML")
  soup = BeautifulSoup(html, "html.parser")
  df_Leitos_compl_nao_SUS_ago = pd.DataFrame(columns=['city_name','UTI_adulto_II_COVID_nao_SUS_ago','UTI_pediatrica_II_COVID_nao_SUS_ago',])
  x = soup.select("td")
  y = soup.select("tr")
  i = 0
  k = 24
  for j in range(4, len(y)):
    ...
    k = k+22
    i = i+1
  df_leitos_comp = df_leitos_comp.merge(df_Leitos_compl_SUS_ago, how='left',on='city_name')
...
def now(config):
  ...
  driver = webdriver.Chrome(chrome_options=chrome_options)  # chromedriver é instalado via dockerfile
  ...
  df_leitos = get_leitos(urlleitos)
  ...
  df_cnes['number_beds'] = df_cnes['cirurgico_tot_ago']+df_cnes['clinico_tot_ago']+df_cnes['hospital_dia_tot_ago']
  df_cnes['number_icu_beds'] = df_cnes['UTI_adulto_I_tot_ago']+df_cnes['UTI_adulto_II_tot_ago']+df_cnes['UTI_adulto_III_tot_ago']
  df_cnes['number_covid_icu_beds'] = df_cnes['UTI_adulto_II_COVID_SUS_ago']+df_cnes['UTI_adulto_II_COVID_nao_SUS_ago']+df_cnes['UTI_pediatrica_II_COVID_SUS_ago']+df_cnes['UTI_pediatrica_II_COVID_nao_SUS_ago']
  # Da merge com os dados de populacao #
  dfs = download_from_drive(config[country]["drive_paths"]["cities_population"])
  df_cnes = pd.merge(df_cnes, dfs,on="city_id",how="left",suffixes=("", "_y"))
  df_cnes = df_cnes.drop(["state_name_y","UTI_pediatrica_II_COVID_nao_SUS_ago","Respiradores_ago","city_name_y","pediatrico_tot_ago","UTI_adulto_II_COVID_SUS_ago","UTI_pediatrica_II_COVID_SUS_ago","UTI_adulto_II_COVID_nao_SUS_ago","state_id_y","cirurgico_tot_ago", "clinico_tot_ago", "hospital_dia_tot_ago", "UTI_adulto_I_tot_ago", "UTI_adulto_II_tot_ago", "UTI_adulto_III_tot_ago"], axis=1)
  todayday = datetime.now().strftime("%Y-%m-%d")
  df_cnes['data_last_refreshed'],df_cnes['last_updated_number_ventilators'],df_cnes['last_updated_number_beds'],df_cnes['last_updated_number_icu_beds'],df_cnes['last_updated_number_covid_icu_beds'],df_cnes["author_number_beds"],df_cnes["author_number_ventilators"],df_cnes["author_number_icu_beds"] = todayday,todayday,todayday,todayday,todayday,"DataSUS","DataSUS","DataSUS"
  return df_cnes

TESTS = {
    "data is not pd.DataFrame": lambda df: isinstance(df_cnes, pd.DataFrame),
    "more than 5570 cities": lambda df: len(df_cnes["city_id"].unique()) <= 5570,
    "no negative beds or icu beds": lambda df: len(
        df_cnes.query("number_beds < 0 | number_icu_beds < 0")
    )
    == 0,
}
