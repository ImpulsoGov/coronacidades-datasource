...
class state_frame:
    ...
    def __init__(
        ...
        sort_ref_vectorized(
            cities_table["state_num_id"].values,
            cities_table["city_name"].values,
            cities_table["city_id"].values,
        )
    ...
    def solve_word(self, wrong_word, correct_word, lev_dis):
        if lev_dis <= 2:
            ...
            try:
                ...
                print("Not found in corrections " + wrong_word)
...
def now(config):
    ...
    raw_inloco_cities = gd.downloadGoogleFileDF(file_id, "token.pickle")
    return state_frame(
