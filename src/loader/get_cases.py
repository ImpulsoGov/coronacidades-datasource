...
def _adjust_subnotification_cases(df, cases_params, config):
    ...
    notification_day = df["last_updated"].max() - datetime.timedelta(
        cases_params["notification_window_days"]
    )
    ...
    city_notif_rate = (
        df.groupby("city_id")
        .apply(lambda x: _get_notification_rate(x, config))
        .reset_index()
    ...
    df["notification_rate"] = np.where(
        df["notification_rate"] == -1,
        df["state_notification_rate"],
        df["city_notification_rate"],
    )

    return df[
        ["city_id", "state_notification_rate", "notification_rate"]
    ].drop_duplicates()
...
def now(country, config):
    ...
    if country == "br":
        ...
        df = df.merge(
            _adjust_subnotification_cases(df, cases_params, config), on="city_id"
        )
